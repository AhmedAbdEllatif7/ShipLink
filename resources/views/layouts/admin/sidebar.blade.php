<aside class="w-64 bg-slate-900 text-white flex flex-col hidden lg:flex transition-all duration-300 shadow-xl z-20">
    ...
    <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        <a href="#" class="flex items-center gap-3 px-4 py-3 bg-indigo-600 rounded-xl font-bold text-white transition-colors shadow-lg shadow-indigo-600/30">
        ...
        <a href="#" class="flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-slate-300 hover:bg-slate-800 hover:text-white transition-colors text-sm">
    ...
    <div class="p-4 border-t border-slate-800">
        <div class="flex items-center gap-3 px-4 py-3 bg-slate-800 rounded-xl">
            <div class="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-sm font-bold text-white">
                {{ Auth::check() ? substr(Auth::user()->name, 0, 1) : 'A' }}
            </div>
            <div>
                <p class="text-sm font-bold text-white">{{ Auth::check() ? Auth::user()->name : 'Admin' }}</p>
                <p class="text-xs text-slate-400">إدارة النظام</p>
            </div>
        </div>
        <form method="POST" action="{{ route('logout') }}" class="mt-2">
            @csrf
            <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm text-red-500 hover:bg-slate-800 hover:text-red-400 rounded-xl transition-colors font-semibold">
